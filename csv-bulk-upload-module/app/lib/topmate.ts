...
export async function fetchProfileByUserId(userId: number): Promise<TopmateProfile> {
  try {
    ...
    try {
      ...
      console.warn(`Topmate API failed for user ${userId}, trying MCP fallback:`, apiError);
    }

    // MCP fallback removed - API-only access
    throw new Error(`Failed to fetch user ${userId} from Topmate API`);
